...
const std::string COMPUTER2_IP = "10.10.59.66";       // Ip of computer 2
...
int connect_socket_2() {
    ...
    serverAddr.sin_addr.s_addr = inet_addr(COMPUTER2_IP.c_str());
    ...
    std::cout << "Connected to server at " << COMPUTER2_IP << ":" << PORT2 << std::endl;
...
void sender() {
    while (1) {
        ...
        // cs
        cout << "accessing critical section\n";
        vector<int> vcS = {4, 5, 6};
        srand(time(0));
        vcS[0] = rand() % 100;
        vcS[1] = 50 + (rand() % 100);
        vcS[2] = 60 + (rand() % 100);
...
int main() {
    ...
    sleep(15);
    connect_socket_cs();
    socket_to_id[socket1] = 1;
    socket_to_id[socket2] = 2;
